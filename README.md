...
### Webpack with [css-modules](https://github.com/css-modules/css-modules) (Recommend!)
```js
import React from 'react';
...
### Some Advise
react-flex-material using [lodash](https://lodash.com/). To minimize size of the bundle, add [babel-plugin-lodash](https://github.com/lodash/babel-plugin-lodash) to your webpack configuration.
```js
{
  test: /\.js$/,
  loader: 'babel',
...
Since this library for React, we are using `camelCased` style instead of `snake-cased`.

Base attributes are `'layout', 'flex', 'order', 'offset', 'align', 'hide', 'show'`

Responsive modifiers `'', 'Xs', 'GtXs', 'Sm', 'GtSm', 'Md', 'GtMd', 'Lg', 'GtLg'`

Other attributes `'layoutWrap', 'layoutNoWrap', 'layoutFill', 'layoutMargin', 'layoutPadding'`

So you'll get `'layoutXs', 'flexGtXs', 'orderSm' ...` 
...
**Important**: use it only if you need a specific alignment.
```jsx
...
### You can use other html tags for Flex
